...
```bash
cargo test --manifest-path fronius-simulator/Cargo.toml
cargo run --manifest-path fronius-simulator/Cargo.toml -- --day-duration-seconds 600
```

The local server listens on http://0.0.0.0:8080 by default.
