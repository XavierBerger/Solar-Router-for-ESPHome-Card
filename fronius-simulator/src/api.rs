...
fn common_inverter_data(sample: &Sample) -> Value {
    ...
    let current_ac = if sample.pv_power_w > 0.0 {
        sample.pv_power_w / 230.0 / 3.0
    } else {
        0.0
    };
    ...
    json!({
        ...
        "DeviceStatus": {
            ...
            "LEDColor": if sample.status == crate::simulation::InverterStatus::Running { 2 } else { 0 },
            "LEDState": 1,
            "StatusCode": sample.status.code()
        }
    })
...
fn three_phase_inverter_data(sample: &Sample) -> Value {
    let phase_current = if sample.pv_power_w > 0.0 {
        sample.pv_power_w / 230.0 / 3.0
    } else {
        0.0
    };
    ...
    json!({
        ...
        "UAC_L2": { "Value": 230.0, "Unit": "V" },
        "UAC_L3": { "Value": 230.0, "Unit": "V" }
    })
}

fn meter_data(sample: &Sample) -> Value {
    let phase_power = sample.grid_power_w / 3.0;
    let phase_current = phase_power.abs() / 230.0;
...
mod tests {
    ...
    #[tokio::test]
    async fn optional_collections_are_empty_but_valid() {
        let storage = get_json("/solar_api/v1/GetStorageRealtimeData.cgi?Scope=System").await;
        let ohmpilot = get_json("/solar_api/v1/GetOhmPilotRealtimeData.cgi?Scope=System").await;
        assert_eq!(storage["Head"]["Status"]["Code"], 0);
        assert_eq!(storage["Body"]["Data"].as_object().unwrap().len(), 0);
        assert_eq!(ohmpilot["Head"]["Status"]["Code"], 0);
        assert_eq!(ohmpilot["Body"]["Data"].as_object().unwrap().len(), 0);
    }
