...
Si l'utilisateur navigue dans le passé, la carte ne doit pas nécessairement forcer immédiatement la vue sur `now`.
...
# 34. Démarrage concret — Phase 1

Cette section constitue le **point de départ opérationnel du développement**.

L'objectif est de pouvoir ouvrir une nouvelle session de développement et commencer directement par les étapes ci-dessous.

## 34.1 État initial connu

Le simulateur existe déjà dans :
...
Elle retourne un `DayData` contenant 1440 échantillons à 60 secondes.

Le frontend n'existe pas encore.

Aucune modification fonctionnelle du simulateur ne doit être faite tant qu'elle n'est pas nécessaire au fonctionnement du viewer.

---

## 34.2 Étape 1 — Vérifier le simulateur

Avant toute modification frontend, vérifier que le simulateur fonctionne.

Lancer l'environnement de développement selon `docs/DEVELOPMENT.md`.

Pour un accès depuis le navigateur, le port du simulateur doit être accessible depuis l'hôte :
...
```text
GET http://localhost:8080/simulation/day
```

Le résultat attendu est un JSON contenant notamment :
...
Cette vérification constitue le **point zéro** avant développement.

---

## 34.3 Étape 2 — Créer le squelette du viewer

Créer :

```text
fronius-simulator/viewer/
├── index.html
├── style.css
├── viewer.js
└── lib/
...
À ce stade, ne pas chercher à construire immédiatement le graphique final.
...
viewer.js
   ↓
fetch("/simulation/day")
   ↓
JSON affiché / inspectable
```

Le premier jalon est donc simplement :

> **Le navigateur récupère correctement les données du simulateur.**

---

## 34.4 Étape 3 — Ajouter uPlot

Ajouter une version locale de uPlot dans :

```text
viewer/lib/uplot/
```

Le viewer doit fonctionner sans CDN.

`index.html` charge ensuite les fichiers locaux nécessaires.

Vérifier qu'un graphique minimal peut être rendu avec une petite série de test.

Avant de connecter les 1440 points du simulateur, valider :
...
## 34.5 Étape 4 — Implémenter l'adaptateur simulateur

Créer une fonction dont la responsabilité est uniquement :

```text
DayData → modèle normalisé
```

Conceptuellement :

```javascript
function adaptSimulatorDay(dayData) {
    return dayData.samples.map(sample => {
        const solar = sample.pv_power_w;
        const consumption = sample.load_power_w;
        const gridPower = sample.grid_power_w;
...
Le renderer ne doit jamais accéder directement à :
...
## 34.6 Étape 5 — Implémenter le renderer

Créer un composant de rendu ayant conceptuellement cette responsabilité :

```text
EnergyChartRenderer
        │
        ├── reçoit le modèle normalisé
        ├── construit le graphique principal
        ├── construit le graphique réseau
        ├── synchronise les curseurs
        └── gère le zoom
```

Il ne doit pas :

- appeler `/simulation/day` ;
- connaître `DayData` ;
- connaître Home Assistant ;
- calculer les flux énergétiques à partir de `grid_power_w`.

---

## 34.7 Étape 6 — Construire le graphique principal
...
Le rendu doit permettre de distinguer :
...
Le premier objectif est la **lisibilité**, pas la finition graphique.

---

## 34.8 Étape 7 — Construire le graphique réseau
...
Il est acceptable de transformer les données uniquement au niveau du renderer pour obtenir cette représentation visuelle, tant que le modèle normalisé conserve :
...
## 34.9 Étape 8 — Synchroniser les graphes

Une fois les deux graphiques fonctionnels :

1. partager la même plage temporelle ;
2. synchroniser le curseur ;
3. synchroniser le zoom ;
4. vérifier le comportement lors d'un redimensionnement.

Cette étape est particulièrement importante pour la future carte Lovelace.

---

## 34.10 Étape 9 — Servir le viewer depuis Rust

Modifier le serveur Axum pour servir les fichiers du viewer.
...
affiche directement le viewer.

L'API continue parallèlement à fonctionner :

```text
http://localhost:8080/simulation/day
```

Le frontend et l'API utilisent donc le même serveur et la même origine.

---

## 34.11 Étape 10 — Tester avec le simulateur accéléré
...
Pour les tests de développement, une valeur réduite permet de faire défiler une journée simulée beaucoup plus rapidement.
...
permet de simuler une journée complète en environ 10 minutes.

Cette fonctionnalité pourra être utilisée pour vérifier ultérieurement le comportement temps réel du viewer, même si la phase 1 reste principalement une visualisation de journée.

---

# 35. Premier jalon de développement

Le **premier objectif concret**, avant toute fonctionnalité avancée, est donc :
...
Une fois ce jalon atteint, on ajoute successivement :
...
tests
        ↓
stabilisation du composant commun
```

**Il ne faut pas commencer la phase 2 avant que cette brique commune soit suffisamment propre et stable.**

---

# 36. Règle de conception pour la suite

À chaque modification effectuée pendant la phase 1, se poser la question :

> **« Est-ce que cette logique appartiendrait également à une future carte Lovelace ? »**

Si oui, elle doit probablement être placée dans une brique générique.

Si elle concerne uniquement :

- `/simulation/day` ;
- `DayData` ;
- le simulateur Fronius ;

elle doit rester dans l'adaptateur du simulateur.

Cette règle doit permettre de conserver une architecture réutilisable sans sur-concevoir la phase 1.

---

# 37. Résultat attendu après les deux phases

À terme, le projet doit avoir une architecture proche de :
...
```text
                   ...
                   ▼                                 ▼
          ┌──────────────────┐              ┌──────────────────┐
          │ Simulator Viewer │              │ Lovelace Card    │
          │                  │              │                  │
          │ SimulatorAdapter │              │ HA Adapter       │
          └────────┬─────────┘              └────────┬─────────┘
                   │                                 │
                   ▼                                 ▼
          /simulation/day                    Home Assistant
```

Le **moteur graphique et le modèle énergétique sont communs**.

Les sources de données et les enveloppes UI sont différentes.

C'est la cible architecturale principale du projet.